...
describe Dashboard::TrainerController do
  describe 'review_card' do
    ...
    it 'calls CheckTranslation' do
      # allow(CheckTranslation).to receive(:call).with(card: @card, user_translation: 'house')
      allow_any_instance_of(CheckTranslation).to receive(:call).with(card: @card, user_translation: 'house')

      put :review_card, params: { card_id: @card.id, user_translation: 'house' }

      expect(CheckTranslation).to have_received(:call)
    end
    ...
    #   it_behaves_like 'correct translation' do
    #     let(:card_factory) { [:card, :second_check, user: @user,
    #                           block: @block] }
